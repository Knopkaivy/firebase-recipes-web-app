...
function App() {
  ...
  async function handleUpdateRecipe(newRecipe, recipeId) {
    try {
      ...
      handleFetchRecipes();
      alert(`successfully updated a recipe with name = ${newRecipe.name}`);
  ...
  async function handleDeleteRecipe(recipeId) {
    ...
    if (deleteConfirmation) {
      try {
        ...
        setCurrentRecipe(null);
        window.scroll(0, 0);
        alert(`Successfully deleted recipe ${recipeId}`);
      } catch (error) {
        alert(error.message);
        throw error;
      }
        ...
        <div className="row filters">
          ...
          <label className="input-label">
            <select
              ...
              className="select"
            >
              <option value="publichDateDesc">
                Publish Date (newest - oldest)
              </option>
              <option value="publichDateAsc">
                ...
                {recipes.map((recipe) => {
                  return (
                    <div className="recipe-card" key={recipe.id}>
                      ...
                      <div className="recipe-field">
                        Published Date: {formatDate(recipe.publishDate)}
                      </div>
                      {user ? (
                        <button
                          type="button"
